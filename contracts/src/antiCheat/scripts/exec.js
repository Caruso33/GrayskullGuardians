...
const main = async () => {
  await arbLog('Cross-chain AntiCheat')
  ...
  /**
   * We deploy L1 AntiCheat to L1, L2 AntiCheat to L2, each with a different "greeting" message.
   * After deploying, save set each contract's counterparty's address to its state so that they can later talk to each other.
   */
  const L1AntiCheat = await (
    await hre.ethers.getContractFactory('AntiCheatL1')
  ).connect(l1Wallet) //
  console.log('Deploying L1 AntiCheat 👋')
  const l1AntiCheat = await L1AntiCheat.deploy(
    // 'Hello world in L1',
    ethers.constants.AddressZero, // temp l2 addr
    inboxAddress
  )
  await l1AntiCheat.deployed()
  console.log(`deployed to ${l1AntiCheat.address}`)

  console.log(`funding l1AntiCheat`)

  await l1AntiCheat.getMoney({ value: BigNumber.from(5000000000000000) })

  const L2AntiCheat = await (
    await hre.ethers.getContractFactory('AntiCheatL2')
  ).connect(l2Wallet)

  console.log('Deploying L2 AntiCheat 👋👋')

  const l2AntiCheat = await L2AntiCheat.deploy(
    // 'Hello world in L2',
    ethers.constants.AddressZero // temp l1 addr
  )
  await l2AntiCheat.deployed()
  console.log(`deployed to ${l2AntiCheat.address}`)

  const updateL1Tx = await l1AntiCheat.updateL2Target(l2AntiCheat.address)
  await updateL1Tx.wait()

  const updateL2Tx = await l2AntiCheat.updateL1Target(l1AntiCheat.address)
  await updateL2Tx.wait()
  console.log('Counterpart contract addresses set in both AntiCheats 👍')

  /**
   * Let's log the L2 greeting string
   */
  // const currentL2Greeting = await l2AntiCheat.greet()
  // console.log(`Current L2 greeting: "${currentL2Greeting}"`)

  console.log('Updating address from L1 to L2:')
  ...
  const newUserAddress = '0xa53A6fE2d8Ad977aD926C485343Ba39f32D3A3F6'
   ...
   */
  const ABI = ['function setAddress(address user)']
  const iface = new ethers.utils.Interface(ABI)
  const calldata = iface.encodeFunctionData('setAddress', [newUserAddress])
  ...
  const RetryablesGasOverrides = {
    gasLimit: {
      ...
      percentIncrease: BigNumber.from(40), // how much to increase the base for buffer
    },
    maxSubmissionFee: {
      base: undefined,
      percentIncrease: BigNumber.from(40),
    },
    maxFeePerGas: {
      base: undefined,
      percentIncrease: BigNumber.from(40),
  ...
  const L1ToL2MessageGasParams = await l1ToL2MessageGasEstimate.estimateAll(
    {
      from: await l1AntiCheat.address,
      to: await l2AntiCheat.address,
      l2CallValue: 0,
      excessFeeRefundAddress: await l2AntiCheat.address,
      callValueRefundAddress: await l2AntiCheat.address,
      data: calldata,
    },
  ...
  console.log(`gas limit ${L1ToL2MessageGasParams.gasLimit.toString()}`)
  const setGreetingTx = await l1AntiCheat.setGreetingInL2(
   ...
   * Thus, when AntiCheatL2 checks that the message came from the L1, we check that the sender is this L2 Alias.
   * See setGreeting in AntiCheatL2.sol for this check.
   */

  /**
   * Now when we call greet again, we should see our new string on L2!
   */
  // const newGreetingL2 = await l2AntiCheat.greet()
  // console.log(`Updated L2 greeting: "${newGreetingL2}" 🥳`)
  const newUint = await l2AntiCheat.getStartTime('0xa53A6fE2d8Ad977aD926C485343Ba39f32D3A3F6')
  console.log("there should be a number here: ", newUint.toString())
}
